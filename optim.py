...
def optim_optax(func, ndim ,x0=None, param_bounds=None, lr=5e-3, maxiter=150, n_restarts=4):
    ...
    if param_bounds is not None:
        ...
        param_bounds = np.array(ndim*[[0,1]]).T
        #scales = 1
    ...
    def step(carry):
        ...
        params = input_unstandardize(u_params, param_bounds)
    ...
    u_params = input_standardize(init_params, param_bounds)

    # display with progress bar
    r = jnp.arange(maxiter)
    for n in range(n_restarts):
        opt_state = optimizer.init(u_params)
        progress_bar = tqdm.tqdm(r, desc=f'Optimising {func.__name__}')
    ...
    params = input_unstandardize(best_params, param_bounds)
    return params, best_f



def optim_optax_vmap_early_term(fn, ndim, x0=None, param_bounds=None, lr=5e-3, maxiter=500, n_restarts_optimiser=4, min_delta=1e-8):
    #Minimum amount by which loss function must improve to be considered progressing
    patience=15 #Number of steps without progress before restart is stopped
    ...
    params = jnp.tile(x0, (n_restarts_optimiser, 1))
    params = input_standardize(params, param_bounds)
    ...
    def scan_step(carry):
        ...
        values, grads = jax.vmap(fn_val_grad)(input_unstandardize(params, param_bounds))
        #grads = grads*scales
        ...
        new_params = jnp.clip(new_params, 0., 1.)
    ...
    best_values, _ = jax.vmap(fn_val_grad)(params)
    ...
    pbar = tqdm_(range(maxiter), desc=f"Optimising {fn.__name__}")
    for i in pbar:
        ...
        if jnp.logical_not(jnp.any(active_mask)):
            break
    best_param = input_unstandardize(best_params[jnp.argmin(best_values)], param_bounds)
    best_f = jnp.min(best_values)
    return best_param, best_f.item() #, best_values