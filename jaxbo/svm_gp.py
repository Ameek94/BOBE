...
class SVM_GP:
    ...
    def __init__(self,support_vectors=None, dual_coef=None, intercept=None, gamma_eff=None,
                 ...
                 svm_acq_flag=True,lengthscales=None,outputscale=1.0,):
        """
    ...
    def update(self,new_x,new_y,refit=True,lr=1e-2,maxiter=150,n_restarts=2,step=0):
        ...
        if jnp.any(jnp.all(jnp.isclose(self.train_x_svm, new_x, atol=1e-6,rtol=1e-4), axis=1)):
            ...
            if (self.svm_data_size > self.svm_use_size) and (step % self.svm_update_step == 0):
                self.update_svm()
            return repeat
    ...
    def gp_numpyro_model(self):
        ...
        This is used for sampling from the GP posterior using NUTS or SA.
        """
        ...
        mean = self.predict_mean(x) #jax.lax.map(self.predict_mean, x,batch_size=2)
    ...
    def get_random_point(self, npoints=1):
        ...
        result = self.train_x_svm[chosen_index]
        log.info(f"Random point sampled with value {self.train_y_svm[chosen_index]}")
    ...
    def sample_GP_NUTS(self, rng_key, warmup_steps=512, num_samples=512, progress_bar=True, thinning=8, verbose=True,
        ...
        if self.use_svm and init_params is not None:
        ...
        if not hmc_success:
            should_restart = True
            if verbose:
                log.info("HMC failed. Restarting with SA kernel.")
        ...
        samples = {
            'x': mc_samples['x'],
            'weights': jnp.ones(mc_samples['x'].shape[0]) ,
