...
class GPwithClassifier:
    def __init__(self, train_x=None, train_y=None, clf_flag=True,
                 ...
                 noise=1e-10, kernel="rbf", optimizer="adam", 
    ...
    def update(self, new_x, new_y, refit=True, lr=5e-3, maxiter=200, n_restarts=4, step=0):
        """
        ...
        if not self.clf_flag:
            ...
            # is_above_threshold = new_y >= self.train_y_clf - self.clf_threshold #(self.predict_mean(new_x) >= self.train_y_clf.max() - self.clf_threshold)
            # if not is_above_threshold:
            #     # check predicted label
            #     predicted_label = (self._clf_predict_func(new_x) >= self.probability_threshold)
            #     correctly_classified = correct_label==predicted_label
    ...
    def reset_train_data(self):
        ...
        if full_size % 100 == 0:
            ...
            train_y_gp = self.train_y_clf[mask]
