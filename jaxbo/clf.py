...
def svm_predict_proba(x: jnp.ndarray, support_vectors: jnp.ndarray, dual_coef: jnp.ndarray, intercept: float, gamma: float):
    ...
    return jnp.where(decision >= 0, 1.0, 0.0)  # Binary classification: 1 if decision >= 0, else 0, can also get continuous score but not really necessary atm
...
def train_with_restarts(
    ...
    **train_kwargs
    ...
    best_metrics.update({'n_restarts': n_restarts})
