...
class external_loglike:
    ...
    def get_initial_points(self,n_init_sobol=8,n_cobaya_init=0):
        ...
        progress_bar = tqdm.tqdm(r, desc="Evaluating Loglikelihood for Sobol points")
        sobol = qmc.Sobol(d=self.ndim, scramble=True).random(n_init_sobol)
        sobol_points = input_unstandardize(sobol,self.param_bounds)
        for i in progress_bar:
            pt = sobol_points[i]
            lp = self.logp(pt)
...
class cobaya_loglike(external_loglike):
    ...
    def __call__(self, x, logp_args = (), logp_kwargs = {}):
        vals =  super().__call__(x, logp_args, logp_kwargs)
        return vals + self.logprior_vol
    
    def get_initial_points(self, n_cobaya_init=4,n_init_sobol=16):
        points, logpost = [], []
        progress_bar = tqdm.tqdm(range(n_cobaya_init), desc='Initial points cobaya ref')
...
# class external_fortran_likelihood(external_loglike):

#     def __init__(self, logposterior: Callable[..., Any],
#                 ndim: int, param_list: List[str] | None = None, 
#                 param_labels: List[str] | None = None,
#                 param_bounds: List | np.ndarray | None = None,
#                 noise_std: float = 0,
#                 name: str | None = None) -> None:
#         super().__init__(logposterior, ndim, param_list, param_labels, param_bounds, noise_std, name)
