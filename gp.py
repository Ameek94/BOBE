...
@jax.jit
def get_mean_from_cho(k11_cho,k12,train_y, prior_mean):
    mu = jnp.matmul(jnp.transpose(k12),cho_solve((k11_cho,True),train_y)) # can also store alphas
    mean = mu.squeeze(-1) + prior_mean
    return mean
    

@jax.jit
def gp_predict(train_y,k11_cho,k12,k22, prior_mean):
    """
    Predicts the GP mean and variance at x
    """
    mean = get_mean_from_cho(k11_cho,k12,train_y, prior_mean)
...
class GP(ABC):
    ...
    def __init__(self,train_x,train_y,noise=1e-8,kernel="rbf",optimizer="adam"
        ...
        self.prior_mean_unstd = prior_mean
        self.prior_mean_std = (self.prior_mean_unstd - self.y_mean)/self.y_std
        self.train_y = (train_y - self.prior_mean_unstd) / self.y_std
    ...
    def predict_mean(self,x):
        ...
        mean = get_mean_from_cho(self.cholesky,k12,self.train_y, self.prior_mean_std)
    ...
    def predict(self,x):
        ...
        k12 = self.kernel(self.train_x,x,self.lengthscales,self.outputscale,noise=self.noise,include_noise=False)
        k22 = self.kernel(x,x,self.lengthscales,self.outputscale,noise=self.noise,include_noise=True)
        mean, var = gp_predict(self.train_y,self.cholesky,k12,k22, self.prior_mean)
        return mean, var
    ...
    def add(self,new_x,new_y):
        ...
        self.train_y = self.train_y*self.y_std + self.prior_mean_unstd #+ self.y_mean
        self.train_y = jnp.concatenate([self.train_y,new_y])
        self.y_mean = jnp.mean(self.train_y,axis=0) #- self.prior_mean
        self.y_std = jnp.std(self.train_y,axis=0)
        self.prior_mean_std = (self.prior_mean_unstd - self.y_mean)/self.y_std
        self.train_y = (self.train_y - self.prior_mean_unstd) / self.y_std
...
class DSLP_GP(GP):
    ...
    def tree_unflatten(cls, aux_data, leaves):
        ...
        #if mean_offset == False:
        #    obj.y_mean = y_mean
        #if mean_offset == 'Const':
        #    obj.y_mean = y_mean - 5
        #if mean_offset == 'ndim':
            #Do something with the Gpry threshold
        obj.y_std = y_std
...
jax.tree_util.register_pytree_node(
    Uniform_GP,
    Uniform_GP.tree_flatten,
    Uniform_GP.tree_unflatten,
)
