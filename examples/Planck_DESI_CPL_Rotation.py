...
def main():
    ...
    likelihood_name = f'Planck_DESI_CPL_Rotation_{seed}'
    ...
    # First, create a temporary BOBE instance to get the parameter order from the likelihood
    print("\nGetting parameter order from likelihood...")
    from BOBE.likelihood import CobayaLikelihood
    temp_likelihood = CobayaLikelihood(cobaya_input_file, minus_inf=-1e5)
    likelihood_param_names = temp_likelihood.param_list
    likelihood_param_bounds = temp_likelihood.param_bounds
    
    print(f"Likelihood parameters: {likelihood_param_names}")
    print(f"Likelihood bounds shape: {likelihood_param_bounds.shape}")
    print(f"Likelihood bounds:\n{likelihood_param_bounds}")
    
    # Get covariance matrix and center using the likelihood's parameter order
    param_indices = [reference_samples.getParamNames().list().index(name) for name in likelihood_param_names]
    print(f"Parameter indices in chains: {param_indices}")
    ...
    center_point = np.array([best_fit.getParamDict()[name] for name in likelihood_param_names])
    print(f"\nBest-fit point (center): {center_point}")
    
    # Verify center is within bounds
    for i, name in enumerate(likelihood_param_names):
        val = center_point[i]
        lower, upper = likelihood_param_bounds[0, i], likelihood_param_bounds[1, i]
        print(f"  {name}: {val:.6f}  (bounds: [{lower:.6f}, {upper:.6f}])")
        if not (lower <= val <= upper):
            print(f"    WARNING: {name} is outside bounds!")
    
    # Clean up temp likelihood
    del temp_likelihood
    ...
    if results is not None:  # when running in MPI mode, only rank 0 returns results, rest return None
        ...
        g.triangle_plot(
            [BOBE_Samples, reference_samples, Gaussian_MCSamples], 
            params=['w','wa','ombh2', 'omch2', 'H0', 'ns', 'logA', 'tau'],
            filled=[True, False, False],
            contour_colors=['#006FED', 'black', 'green'], 
            contour_lws=[1, 1.5, 1.5],
            legend_labels=['BOBE (rotated)', 'MCMC', 'Gaussian'],
        ) 
