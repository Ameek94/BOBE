...
def main():
    ...
    likelihood_name = f'Planck_DESI_LCDM_r_{seed}'
    ...
    reference_samples = loadMCSamples(
            './cosmo_input/chains/Planck_DESI_BK18',
            settings={'ignore_rows': 0.3, 'label': 'MCMC'}
        )
    
    # cov_matrix = np.loadtxt('./results/LCDM_r/Planck_DESI_CPL_Fisher_100_cov_matrix.txt')
    # center_point = np.loadtxt('./results/LCDM_r/Planck_DESI_CPL_Fisher_100_cov_peak.txt')
    ...
    bobe = BOBE(
        ...
        resume=True,
        resume_file=f'./results/LCDM_r/{likelihood_name}',
        save_dir='./results/LCDM_r/',
        save=True,
        verbosity='INFO',
        n_cobaya_init=16,
        n_sobol_init=64,
    ...
    if results is not None:  # when running in MPI mode, only rank 0 returns results, rest return None
        ...
        g.export(f'./results/LCDM_r/{likelihood_name}_cosmo_samples.pdf')
        print(f"Saved plot to ./results/LCDM_r/{likelihood_name}_cosmo_samples.pdf")
        ...
        g.export(f'./results/LCDM_r/{likelihood_name}_full_samples.pdf')
        print(f"Saved plot to ./results/LCDM_r/{likelihood_name}_full_samples.pdf")
        ...
        plt.savefig(f"./results/LCDM_r/{likelihood_name}_acquisition.pdf", bbox_inches='tight')
        print(f"Saved acquisition plot to ./results/LCDM_r/{likelihood_name}_acquisition.pdf")
