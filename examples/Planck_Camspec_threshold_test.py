...
threshold = float(sys.argv[1]) if len(sys.argv) > 1 else 300

likelihood = cobaya_likelihood(cobaya_input_file, confidence_for_unbounded=0.9999995,
        minus_inf=-1e5, noise_std=0.0,name=f'Planck_Camspec_fast_{threshold}')
...
sampler = BOBE(n_cobaya_init=16, n_sobol_init = 128, 
        ...
        lengthscale_priors='DSLP',logz_threshold=25.,
        use_svm=True,svm_use_size=350,svm_update_step=5,minus_inf=-1e5,
        svm_threshold=threshold,svm_gp_threshold=5000.) #prev 5000
