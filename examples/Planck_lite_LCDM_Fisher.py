...
def main():
    ...
    likelihood_name = f'Planck_lite_LCDM_Fisher'
    ...
    if results is not None:  # when running in MPI mode, only rank 0 returns results, rest return None
        ...
        samples_array = np.random.multivariate_normal(mean=peak, cov=np.linalg.inv(fisher_matrix), size=4096)
        ...
        g.triangle_plot([fisher_samples,reference_samples], params=['ombh2','omch2','H0','ns','logA','tau'],
                    ...
                    legend_labels=['BOBE Fisher', 'Nested Sampling'],) 
