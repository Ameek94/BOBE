"""
Planck TT/EE + ACT DR6 (Planck-ACT cut + ACT-only) LCDM+nrun script with
continuous GP parameter rotation applied to cosmological parameters only.
Nuisance parameters (A_planck, P_act) and the spectral index running nrun are
kept in a simple linear (axis-aligned) unit-cube mapping.  Note: A_act is
derived from A_planck and is not a free parameter.
...
PACT_NRUN_COSMO_PARAMS = ['ombh2', 'omch2', 'cosmomc_theta', 'logA', 'ns', 'tau']
...
def main():
    ...
    cobaya_input_file = './cosmo_input/p_actlite_nrun.yaml'
    likelihood_name = f'PActlite_nrun_Rotation_cosmo_continuous_adaptive_batch_{seed}'

    print("Loading reference samples...")
    reference_samples = loadMCSamples(
        './cosmo_input/chains/p-actlite_nrun_camb/p-actlite_nrun_camb',
    ...
    active_dims = [likelihood.param_list.index(p)
                   for p in PACT_NRUN_COSMO_PARAMS if p in likelihood.param_list]
    ...
    print(f"inactive params : {[p for p in likelihood.param_list if p not in PACT_NRUN_COSMO_PARAMS]}")
    ...
    bobe = BOBE(
        ...
        resume=True,
        resume_file=f'./results/PActlite_nrun/{likelihood_name}',
        save_dir='./results/PActlite_nrun/',
        save=True,
        verbosity='INFO',
        n_cobaya_init=4,
        n_sobol_init=32,
    ...
    if results is not None:
        ...
        g.triangle_plot(
            ...
            params=PACT_NRUN_COSMO_PARAMS,
            filled=[True, False],
            contour_colors=['#006FED', 'black'],
            contour_lws=[1, 1.5],
            legend_labels=['BOBE (cosmo rotation, adaptive batch)', 'MCMC'],
        )
        g.export(f'./results/PActlite_nrun/{likelihood_name}_cosmo_samples.pdf')
        print(f"Saved plot to ./results/PActlite_nrun/{likelihood_name}_cosmo_samples.pdf")
        ...
        g.export(f'./results/PActlite_nrun/{likelihood_name}_full_samples.pdf')
        print(f"Saved plot to ./results/PActlite_nrun/{likelihood_name}_full_samples.pdf")
        ...
        plt.savefig(f"./results/PActlite_nrun/{likelihood_name}_acquisition.pdf", bbox_inches='tight')
        print(f"Saved acquisition plot to ./results/PActlite_nrun/{likelihood_name}_acquisition.pdf")
