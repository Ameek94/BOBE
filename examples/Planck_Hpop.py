...
likelihood = cobaya_likelihood(cobaya_input_file, confidence_for_unbounded=0.9999995,
        minus_inf=-1e5, noise_std=0.0,name='Planck_Hpop')

start = time.time()
sampler = BOBE(n_cobaya_init=16, n_sobol_init = 256, 
        miniters=500, maxiters=4000,max_gp_size=2000,
        loglikelihood=likelihood,
        resume=False,
        resume_file=f'{likelihood.name}.npz',
        save=True,
        fit_step = 25, update_mc_step = 15, ns_step = 100,
        num_hmc_warmup = 512,num_hmc_samples = 512, mc_points_size = 64,
        lengthscale_priors='DSLP',logz_threshold=5.,
        use_svm=True,svm_use_size=500,svm_update_step=5,minus_inf=-1e6,svm_threshold=300,svm_gp_threshold=10000)
