...
samples_x = loadMCSamples('./results/LCDM/Best/Planck_DESI_LCDM_z025')
...
def main():
  ...
  for seed in seeds:
      for rotated in [False, True]:
          ...
          likelihood_name = f"Planck_DESI_LCDM_{'rotated_' if rotated else ''}_z05_s{seed}_v2"
          ...
          bobe = BOBE(
              ...
              minus_inf=-1e5,
              seed=seed,
              gp_kwargs={'rotation_samples': samples_x.samples, 'rotation_logwt': None, 'rotation_logl': samples_x.loglikes, 'learn_rotation': False, 'rotation_top_frac': 1.0, 'WIPStd_rotation_threshold': -1} if rotated else {},
          )
          ...
          if results is not None:  # when running in MPI mode, only rank 0 returns results, rest return None
              ...
              g.triangle_plot([BOBE_Samples,reference_samples], filled=[True, False],
                          contour_colors=['#006FED', 'black'], contour_lws=[1, 1.5],
                          legend_labels=['BOBE', 'MCMC']) 
              g.export(f'./rotation_tests/{likelihood.name}_full_posteriors.pdf')
              ...
              plt.savefig(f"./rotation_tests/{likelihood.name}_acquisition.pdf", bbox_inches='tight')
