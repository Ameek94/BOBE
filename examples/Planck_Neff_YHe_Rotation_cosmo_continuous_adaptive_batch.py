"""
Planck lite (TT + EE_sroll2 + TTTEEE_lite_native) LCDM + Neff + YHe script
with continuous GP parameter rotation applied to the base cosmological
parameters only.  The extensions nnu (N_eff) and YHe are kept in a simple
...
All remaining sampled parameters (nnu, YHe) use linear scaling.
"""
...
# Extension parameters (nnu, YHe) remain linear.
PLANCK_NEFF_YHE_COSMO_PARAMS = ['ombh2', 'omch2', 'cosmomc_theta', 'logA', 'ns', 'tau']
...
def main():
    ...
    cobaya_input_file = './cosmo_input/planck_neff_yhe.yaml'
    likelihood_name = f'Planck_Neff_YHe_Rotation_cosmo_continuous_adaptive_batch_{seed}'

    print("Loading reference samples...")
    reference_samples = loadMCSamples(
        './cosmo_input/chains/planck_neff-hed+yhe_camb/planck_neff-hed+yhe_camb',
    ...
    active_dims = [likelihood.param_list.index(p)
                   for p in PLANCK_NEFF_YHE_COSMO_PARAMS if p in likelihood.param_list]
    ...
    print(f"inactive params : {[p for p in likelihood.param_list if p not in PLANCK_NEFF_YHE_COSMO_PARAMS]}")
    ...
    bobe = BOBE(
        ...
        resume=True,
        resume_file=f'./results/Planck_Neff_YHe/{likelihood_name}',
        save_dir='./results/Planck_Neff_YHe/',
        save=True,
        verbosity='INFO',
        n_cobaya_init=4,
        n_sobol_init=32,
    ...
    if results is not None:
        ...
        g.triangle_plot(
            ...
            params=PLANCK_NEFF_YHE_COSMO_PARAMS + ['nnu', 'YHe'],
            filled=[True, False],
            contour_colors=['#006FED', 'black'],
            contour_lws=[1, 1.5],
            legend_labels=['BOBE (cosmo rotation, adaptive batch)', 'MCMC'],
        )
        g.export(f'./results/Planck_Neff_YHe/{likelihood_name}_cosmo_samples.pdf')
        print(f"Saved plot to ./results/Planck_Neff_YHe/{likelihood_name}_cosmo_samples.pdf")
        ...
        g.export(f'./results/Planck_Neff_YHe/{likelihood_name}_full_samples.pdf')
        print(f"Saved plot to ./results/Planck_Neff_YHe/{likelihood_name}_full_samples.pdf")
        ...
        plt.savefig(f"./results/Planck_Neff_YHe/{likelihood_name}_acquisition.pdf", bbox_inches='tight')
        print(f"Saved acquisition plot to ./results/Planck_Neff_YHe/{likelihood_name}_acquisition.pdf")
