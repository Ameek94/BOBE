...
def main():
    ...
    likelihood_name = f'Planck_DESI_CPL_Fisher_{seed}'

    start = time.time()
    print("Starting BOBE run...")

    # Pass Cobaya YAML file path directly to BOBE
    fisher_estimator = Fisher(
    ...
    if results is not None:  # when running in MPI mode, only rank 0 returns results, rest return None
        ...
        fisher_matrix = results['fisher_matrix']
        peak = results['fisher_peak']
        samples_array = np.random.multivariate_normal(mean=peak, cov=np.linalg.inv(fisher_matrix), size=16000)
        samples_array = np.clip(samples_array, param_bounds[0, :], param_bounds[1, :])  # Ensure samples are within bounds
        fisher_samples = MCSamples(samples=samples_array, names=param_list, labels=param_labels,
        ...
        g.triangle_plot([fisher_samples,reference_samples], params=['w', 'wa','ombh2','omch2','H0','ns','logA','tau'],
                        filled=[True, False],
                    contour_colors=['#006FED', 'black'], contour_lws=[1, 1.5],
                    legend_labels=['BOBE Fisher', 'MCMC'],) 
        ...
        g.export(f'./results/CPL/{likelihood.name}_cosmo_samples.pdf')
        ...
        # plt.savefig(f"./results/CPL/{likelihood.name}_acquisition.pdf", bbox_inches='tight')
