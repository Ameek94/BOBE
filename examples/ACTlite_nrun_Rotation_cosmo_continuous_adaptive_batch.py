"""
ACT DR6 + Planck EE_sroll2 LCDM+nrun script with continuous GP parameter
rotation applied to cosmological parameters only.  ACT nuisance parameters
(A_act, P_act) and the spectral index running nrun are kept in a simple linear
(axis-aligned) unit-cube mapping.
...
All remaining sampled parameters (A_act, P_act, nrun) use linear scaling.
"""
...
ACT_NRUN_COSMO_PARAMS = ['ombh2', 'omch2', 'cosmomc_theta', 'logA', 'ns', 'tau']
...
def main():
    ...
    cobaya_input_file = './cosmo_input/actlite_nrun.yaml'
    likelihood_name = f'ACTlite_nrun_Rotation_cosmo_continuous_adaptive_batch_{seed}'

    print("Loading reference samples...")
    reference_samples = loadMCSamples(
        './cosmo_input/chains/actlite_nrun_camb/actlite_nrun_camb',
    ...
    active_dims = [likelihood.param_list.index(p)
                   for p in ACT_NRUN_COSMO_PARAMS if p in likelihood.param_list]
    ...
    print(f"inactive params : {[p for p in likelihood.param_list if p not in ACT_NRUN_COSMO_PARAMS]}")
    ...
    bobe = BOBE(
        ...
        resume=True,
        resume_file=f'./results/ACTlite_nrun/{likelihood_name}',
        save_dir='./results/ACTlite_nrun/',
        save=True,
        verbosity='INFO',
        n_cobaya_init=8,
        n_sobol_init=32,
    ...
    if results is not None:
        ...
        g.triangle_plot(
            ...
            params=ACT_NRUN_COSMO_PARAMS+['nrun'],
            filled=[True, False],
            contour_colors=['#006FED', 'black'],
            contour_lws=[1, 1.5],
            legend_labels=['BOBE (cosmo rotation, adaptive batch)', 'MCMC'],
        )
        g.export(f'./results/ACTlite_nrun/{likelihood_name}_cosmo_samples.pdf')
        print(f"Saved plot to ./results/ACTlite_nrun/{likelihood_name}_cosmo_samples.pdf")
        ...
        g.export(f'./results/ACTlite_nrun/{likelihood_name}_full_samples.pdf')
        print(f"Saved plot to ./results/ACTlite_nrun/{likelihood_name}_full_samples.pdf")
        ...
        plt.savefig(f"./results/ACTlite_nrun/{likelihood_name}_acquisition.pdf", bbox_inches='tight')
        print(f"Saved acquisition plot to ./results/ACTlite_nrun/{likelihood_name}_acquisition.pdf")
