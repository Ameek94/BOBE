...
cosmo = sampler(cobaya_model=True,input_file=input_file,seed=200000,
                max_steps=max_steps, nstart=nstart,mc_points_size=32,acq_goal=1e-12,noise=1e-5)
...
samples = sample_GP_NUTS(gp = cosmo.gp,rng_key=rng_key,num_warmup=2048,num_samples=8192,thinning=4)

np.savetxt('samples_2.txt',samples)
...
g.triangle_plot([gp_samples_nuts,mcmc_samples], keys,filled=[True,False],contour_lws=[1,1.5,1.],
                  ...
                  marker_args={'lw': 1.25, 'ls': '-.', 'color': 'C2'},legend_labels=[f'GP fit, N = {train_x.shape[0]} samples','Cobaya MCMC']) #,param_limits=cosmo.bounds_dict) # type: ignore
