"""
ACT DR6 + BICEP/Keck 2018 + Planck EE_sroll2 LCDM+r script with continuous GP
parameter rotation applied to cosmological parameters only.  ACT nuisance
parameters (A_act, P_act) and the tensor-to-scalar ratio r are kept in a
simple linear (axis-aligned) unit-cube mapping.
...
ACT_BK_R_COSMO_PARAMS = ['ombh2', 'omch2', 'cosmomc_theta', 'logA', 'ns', 'tau']
...
def main():
    ...
    cobaya_input_file = './cosmo_input/actlite_bk_r.yaml'
    likelihood_name = f'ACTlite_BK_r_Rotation_cosmo_continuous_adaptive_batch_{seed}'

    print("Loading reference samples...")
    reference_samples = loadMCSamples(
        './cosmo_input/chains/actlite-bk_r_camb/actlite-bk_r_camb',
    ...
    active_dims = [likelihood.param_list.index(p)
                   for p in ACT_BK_R_COSMO_PARAMS if p in likelihood.param_list]
    ...
    print(f"inactive params : {[p for p in likelihood.param_list if p not in ACT_BK_R_COSMO_PARAMS]}")
    ...
    bobe = BOBE(
        ...
        resume=True,
        resume_file=f'./results/ACTlite_BK_r/{likelihood_name}',
        save_dir='./results/ACTlite_BK_r/',
        save=True,
        verbosity='INFO',
        n_cobaya_init=8,
        n_sobol_init=32,
    ...
    if results is not None:
        ...
        cosmo_params = ['r'] + ACT_BK_R_COSMO_PARAMS
        g.triangle_plot(
        ...
        g.export(f'./results/ACTlite_BK_r/{likelihood_name}_cosmo_samples.pdf')
        print(f"Saved plot to ./results/ACTlite_BK_r/{likelihood_name}_cosmo_samples.pdf")
        ...
        g.export(f'./results/ACTlite_BK_r/{likelihood_name}_full_samples.pdf')
        print(f"Saved plot to ./results/ACTlite_BK_r/{likelihood_name}_full_samples.pdf")
        ...
        plt.savefig(f"./results/ACTlite_BK_r/{likelihood_name}_acquisition.pdf", bbox_inches='tight')
        print(f"Saved acquisition plot to ./results/ACTlite_BK_r/{likelihood_name}_acquisition.pdf")
