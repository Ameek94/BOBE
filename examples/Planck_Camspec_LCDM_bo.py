...
input_dict = {
    ...
    "sampler": {
        "mcmc": {
            ...
            "proposal_scale": 1.9,
            "covmat": "./chains/Planck_DESI_LCDM.covmat",
            "Rminus1_stop": 0.01,
            "Rminus1_cl_stop": 0.2,
        },
    },
...
likelihood = cobaya_loglike(input_dict, confidence_for_unbounded=0.9999995,
        minus_inf=-1e5, noise_std=0.0,name=f'Planck_Camspec_LCDM_{num_fixed_nuisance_params}')

n_sobol_init = 32 if likelihood.ndim < 10 else 128
fit_step = 16 if likelihood.ndim < 10 else 32
update_mc_step = 5 if likelihood.ndim < 10 else 10
svm_threshold = 250 * sqrt(likelihood.ndim / 15)

start = time.time()
sampler = BOBE(n_cobaya_init=16, n_sobol_init = n_sobol_init, 
        miniters=150, maxiters=1600,max_gp_size=1350,
        loglikelihood=likelihood,
        resume=False,
        resume_file=f'{likelihood.name}.npz',
        save=True,
        fit_step = fit_step, update_mc_step = update_mc_step, ns_step = 100,
        num_hmc_warmup = 512,num_hmc_samples = 512, mc_points_size = 64,
        lengthscale_priors='DSLP',logz_threshold=10.,svm_threshold=svm_threshold,svm_gp_threshold=5000,
...
plot_params = ['omch2','logA','ns','H0','ombh2','tau']
plot_final_samples(gp, ns_samples,param_list=sampler.param_list,param_bounds=sampler.param_bounds,
                   ...
                   reference_file=f'./cosmo_input/chains/Planck_DESI_LCDM_{num_fixed_nuisance_params}',reference_ignore_rows=0.3,
